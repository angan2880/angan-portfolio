...
const CLIPPINGS_DB_ID = process.env.NOTION_CLIPPINGS_DATABASE_ID;
...
export async function getClippingsFromNotion(limit) {
  try {
    const query = {
      database_id: CLIPPINGS_DB_ID,
      filter: { property: 'Published', checkbox: { equals: true } },
      sorts: [{ property: 'Date', direction: 'descending' }],
    };
    if (limit) query.page_size = limit;
    ...
    console.error('Error fetching clippings from Notion:', error);
    return [];
  }
}

export async function getClippingByIdFromNotion(id) {
  try {
    ...
    console.error(`Error fetching clipping with ID "${id}" from Notion:`, error);
    return null;
  }
}
