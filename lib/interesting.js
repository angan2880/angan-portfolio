import { getInterestingItemsFromNotion, getInterestingItemByIdFromNotion } from './notion';
...
/**
 * Returns all interesting items, preferably from Notion
...
export async function getAllInterestingItems(limit) {
  try {
    ...
    const notionItems = await getInterestingItemsFromNotion(limit);

    // If we got data from Notion, return it
    if (notionItems && notionItems.length > 0) {
      return notionItems;
    }

    // If no data from Notion, fall back to local data
...
/**
 * Returns a single interesting item by ID, preferably from Notion
...
export async function getInterestingItemById(id) {
  try {
    // First try to get data from Notion
    const notionItem = await getInterestingItemByIdFromNotion(id);
